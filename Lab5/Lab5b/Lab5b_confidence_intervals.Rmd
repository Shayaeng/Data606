...
In this sample, 67% of the population thinks that climate change affects
their local community. This is slightly higher than the 62% in the total
population.
...
**Insert your answer here** I would expect another student's sample to
be similar but different to mine. Similar because we are both sampling
the same total population and expect each of our samples to closely
match that total. Different because it was randomly generated and can be
a different sample for everyone the same way my sample is different from
...
95% confidence means that out of 100 such samples we expect 95 of them
to give us a true confidence interval. We expect to be wrong in the
remaining 5 instances but are willing to accept that.
...
The true proportion of the entire population does indeed fall within the
range provided by this sample of 60. The range I got had a lower limit
of 0.55 and an upper limit of 0.78. 0.62 is within that.
...
I would expect 95% of the different results to fall within the true
population mean. That is due to this model having a 95% confidence
interval.
...
When I ran the app using the above parameters, I got a success rate of
around 93%. That is lower than the confidence level i wanted but pretty
close. The reason for that difference is that the confidence level is
not an exact science. It is what we expect to happen not what will
always happen. The larger the sampling the less that should fluctuate. I
do not know how to create that plot.

```{r}
#I created a data frame to plot
ci_df <- data.frame(lower_ci = numeric(0), upper_ci = numeric(0))
...
# I cannot figure out how to create that plot
#ggplot(ci_df) +
        #geom_segment(aes(x = x_lower, y = y_lower, xend = x_upper, yend = y_upper, color = capture_p)) +
        #geom_point(aes(x = x_lower, y = y_lower, color = capture_p)) +
        #geom_point(aes(x = x_upper, y = y_upper, color = capture_p)) +
...
The higher the confidence level, the wider the the intervals. This is
the case because as the confidence level approaches 100% the total
amount of possible solutions it contains has to grow to approach all
possible solutions. Therefore, a confidence level above 95% would be
wider than the 95% and one lower would be narrower.
...
To achieve a confidence level of 99%, the confidence intervals were
0.517 to 0.817. We are 99% confident that the population ratio will fall
in that range.
...
When I run the app at 99% confidence level, 50 confidence intervals, and
100 bootstraps, the proportion of intervals that include the true
population proportion is 98%. It is slightly lower than the confidence
level but close enough. Again, I do not know how to include the plot.
...
Trying another confidence level of 90%, the confidence interval is
(0.567, 0.767) which means that we are 90% confident that the population
mean will lie within the interval. We expect this interval to be
narrower as it has a smaller confidence level. Interestingly, while
using the app for 90% confidence level, the proportion of intervals that
capture the true population proportion was 94%.
...
A larger sample size results in a narrower range while a smaller one
results in a wider range. This is due to a larger sample always being
more indicative of the reality and therfore being much more likely to
...
The width seems to decrease the fewer times you bootstrap. The standard
error rises as you lower the bootstrap samples.
