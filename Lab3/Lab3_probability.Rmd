...
**Insert your answer here** A streak of one means he made one shot
followed by a miss. So a total of two shots are being counted. A streak
of zero means that either the first of all shots or the first shot
following a miss was another miss. (In essence, that's usually two, or
more, missed shots ina a row).
...
```{r calc-streak-kobe}
kobe_streak <- data.frame(calc_streak(kobe_basket$shot))
...
The plot above shows us the distribution of Kobe's streaks. He had 24
streaks of 1 made shot, 6 streaks of both 2 and 3 made shots and one
streak of 4 made shots in a row. The majority of "streaks" were 0.
Meaning, he missed a shot following a different miss. His most frequent
streks of made shots (minimum 1) was of one made shot. The plot does
seem to show that once he made 2 shots in a row he was much more likely
to make a third ( greater than 50% in this sample) to make at least one
more.
...
set.seed(1125)
coin_outcomes <- c("heads", "tails")
unfair_coin_sim <- sample(coin_outcomes, size = 100, replace = TRUE, prob = c(0.2, 0.8))
table(unfair_coin_sim)


```

In my simulation, heads comes up 27 times and tails comes up 73 times.
...
The above plot shows a pretty straightforward picture. The amount of
streaks of zero makes is 40, exactly double the amount of streaks with
exactly one makes. The amount of streaks of 2 is slightly less than half
of that at 8. This is followed by streaks of 3 makes at 5 and finally
streaks of four makes happened twice. It is pretty easy to see the
independence of each shot. The likelihood of extending a streak seems to
be roughly the same as starting a new one. The differences to start to
fluctuate as the streaks get larger mostly because if the smaller sample
sizes as we go along.
...
If I were to run the simulation again for the simulated independent
shooter I would expect to have pretty similar distributions. Obviously
not the exact same due to the variability involved in making any given
shot. (Assuming using a different seed). But pretty similar due to the
independence of the shots. As noted earlier, as we deal with the longer
streaks, the sample size gets smaller and could lead to wilder swings.
...
```{r}
sim_streak$source <- "Simulated"
kobe_streak$source <- "Kobe"
combined_data <- rbind(sim_streak, kobe_streak)

ggplot(data = combined_data, aes(x = length)) +
  geom_bar() +
  facet_grid(. ~ source) +
  labs(title = "Streak Length Distribution")

```

By plotting the two graphs side by side, we can easily see a difference.
The distribution in the first graph is much less linear than in the
second graph. In the first graph we see a drastic correlation in the
likelihood of a streak of 3 makes relative to 2 makes. This implies that
if he gets 'hot' and hits 2 shots in a row, he is much more likely to
make a third one than he is to make one after a miss. In the second
graph this relationship doesn't exist. The first graph is Kobe's streak
length distribution and implies that the 'hot hands' theory does hold
some water and that the variables are dependant on each other. The
second graph is independent and therefore has no relationship.
