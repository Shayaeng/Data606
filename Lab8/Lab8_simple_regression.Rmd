...
```{r}
#not using glimpse() because there are a lot of columns and don't want so much information
dim(hfi)
```
...
*The appropiate plot would be a scatterplot. There does seem to be a
linear relationship between the variable sand I therefore would be
comfortable using a linear model to predict the personal freedom score
using the ploitical pressure scores as a predictive variable.*
...
**Insert your answer here** *The plot shows that the relationship looks
positively linear. By using the above code to calculate the probability,
we can see the correlation is fairly strong at 0.8. This can aso be seen
in the scatterplot.*
...
**Insert your answer here** *After running the function several times,
the smallest sum of residuals I got was 982. (There was something wrong
with my interactive viewer and the line was drawn above the two places I
clicked, so I had to estimate based on prior results where to draw the
line to get the lowest possible result)*
...
*Using the estimates from the above output we can calculate the
regression line as
:*$$\hat{y} = 5.153687 + 0.349862 \times pf\_expression\_control$$

*The slope of 0.349862 tells us there is a positive correlation, and for
every increase of one in the variable pf_expression_control, we should
expect an increase of 0.349862 in the predicted value of the hf_score.*
...
```{r}
pf_expression_control_v <- 6.7
pf_score_v <- 4.61707 + 0.49143 * pf_expression_control_v
(pf_score_v)
```

```{r}
#there are no 6,7 observed variables so I am expanding the parameters
average_residual <- hfi %>%
  filter(pf_expression_control >= 6.65 & pf_expression_control <= 6.75) %>%
  select(pf_expression_control, pf_score) %>%
  mutate(residual = pf_score - 7.909651) %>%
  summarize(average = mean(residual))

(average_residual)
```

*The predicted pf_score for a pf_expression_control of 6.7 would be
7.909651. Since there aren't any datapoints for a pf_score of 6.7, it is
hard to know if that is an overestimate or underestimate. The closest
datapoints we have are for a pf_expression_control of 6.75, so I
calculated the residuals of 6.65-6.75 (in reality this is just 6.75
since there aren't any datapoints at 6.65 either). The average residual
for the existing datapoints was 0.0967. So the estimated pf_score was an
underestimate by that amount.*
...
*There does not seem to be a pattern to the residuals. They also seem to
be fairly normally distributed. Based on those observations, we can
fairly confidentially say there is a linear relationship between the two
variables.*
...
*Based on the above plots, the nearly normal residual condition is
indeed met.*
...
*I believe the constant variability condition is met, but there does
seem to be more extreme outliers towards either end of the plot.*
...
```{r}
ggplot(hfi, aes(x=ef_legal_integrity, y=ef_legal_courts)) +
  geom_point() +
  theme_minimal() +
  stat_smooth(method = "lm", se = FALSE)
```

*There does seem to be a distinct linear relationship between the
integrity of the legal system and having impartial courts.*
...
```{r}
m3 <- lm(ef_legal_integrity ~ ef_legal_courts, data = hfi)
summary(m1)
summary(m3)
```

The $R^2$ value of my model is 0.3608, which means the model accurately
fits 36.08% of the datapoints, while the $R^2$ value of the model
comparing the relationship between pf_expression_control and pf_score is
0.6342, which means it accurately fits 63.42% of the datapoints. This
means that the latter model does a much better job predicting the
...
*The below plots clearly show a distinct linear relationship between a
country's hf_score and it's pf_score. What surprises me however, was the
relatively low* $R^2$ *value. The* $R^2$ *value is 0.4006 which means
this model only accurately captures 40.06% of the observed datapoints
which is much lower than I would have predicted.*
