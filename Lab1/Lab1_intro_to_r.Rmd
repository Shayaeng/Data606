...
```{r glimpse-data-result, echo=FALSE, message=TRUE, results="asis"}
glimpse(arbuthnot)
```
...
```{r plotpoint-girls-vs-year, message=TRUE}
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_point()
```

There seems to be an overall trend of rising baptism rates for girls.
However, something external presumably caused a huge crash in baptisms between the year 1640 and 1660.
This is evidenced by the extreme drop in baptism rates.
(It is worth noting that after the initial crash culminating in 1650, rates started climbing again for 6 years and then quickly crashed back down before starting climbing again until the end of the collected data. This last aspect however, is a very small sample size and might just be an anomaly and not related to the larger downturn.)

As a whole though, we can assume baptism rates for girls continue rising unless something external has a major impact.
...
```{r plot-prop-boys-vs-year}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_point()

```

The proportion of boys born relative to girls in the observed data is consistently above 50%.
This means that, for whatever reason, more boys were being baptized (and presumably born) than girls in any given year observed.
Other than that finding though, there doesn't seem to be any pattern in regards to how much above 50% that ratio would be.
This implies that, other than confidently assuming the proportion will be greater than half, there is no way to predict what it would be going forward.
...
```{r summarize years-range}
present %>%
  summarize(min = min(year), max = max(year))
```

```{r glimpse dimensions}
glimpse(present)

```

The present dataset starts tracking baptism rates in the year 1940 and ends in the year 2002.
It has a total of 63 rows and 3 columns.
The three variable names included in the dataset are; year, boys and girls.
We can however, use the same script as earlier to add those variables to have a better matching table to compare to.

```{r present-calc-total-bapt-vars-save}
present <- present %>%
  mutate(total = boys + girls)
```

```{r present-calc-prop-boys-to-girls-vars}
present <- present %>%
  mutate(boy_to_girl_ratio = boys / girls)
```

```{r present-calc-prop-boys-vars}
present <- present %>%
  mutate(boy_ratio = boys / total)
```

```{r present-boys-more-than-girls}
present <- present %>%
  mutate(more_boys = boys > girls)
```

Our table now consists of the same seven variables that the arbuthnot table did; years, boys, girls, total, boy to girl ratio, boy ratio and whether there were more boys than girls or not.
The present dataset does collect data from fewer years though.
This is evidenced by the smaller amount of rows, 63 to 82.

1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

As mentioned, the amount of years observed was slightly less in the present dataset.
However, the total amount of data, in this case children being baptized, was much higher in the present dataset.
This makes the patterns and observations more reliable.
...
```{r present-plot-prop-boys-vs-year}
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_point()
```

```{r summarize min and max-arbuthnot-ratio}
arbuthnot %>%
  summarize(min = min(boy_ratio), max = max(boy_ratio))
```

```{r summarize min and max-present-ratio}
present %>%
  summarize(min = min(boy_ratio), max = max(boy_ratio))

```

The dataplot does show that Arbuthnot's observation of boys being born in greater proportion is true as that remains the case in the present dataset.
Interestingly, the actual proportion, while remaining above 50% at all times does not seem to fluctuate as much and stays in a much tighter range.
It also does seem like it os in a downwards trend with one catalyst in middle bringing it back up before continuing it's downtrend.
