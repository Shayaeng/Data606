...
```{r}
dq_mcd <- rbind(dairy_queen, mcdonalds)

ggplot(dq_mcd, aes(x = cal_fat)) +
  geom_histogram() + 
  labs(title = "Calories From Fat in Various Fast Food Chains") +
  xlab("Calories from Fat") + ylab("Items") +
  scale_x_continuous(breaks = seq(0, 1500, by = 200)) +
  facet_wrap(~ restaurant, ncol = 2)
```

The two restaurants have very similar distributions, with the majority
of menu items having less than 400 calories from fat. There are some
noticeable differences though. The Dairy Queen menu seems to have more
lower calorie options, as indicated by the highest bar being earlier,
and less, or none, extremely high calorie options. The distribution
appears to b relatively normal by both restaurants other than some
extreme outliers in the McDonald's menu.
...
It does follow a 'nearly' normal distribution. However, it has a bit of
a right skew and a higher density towards the mean.
...
sim_df <-data.frame(sim_norm) %>%
  rename(cal_fat = sim_norm)

ggplot(data = sim_df, aes(sample = cal_fat)) + 
  geom_line(stat = "qq")
```

```{r}
ggplot() +
  geom_line(data = sim_df, aes(sample = cal_fat, color = "blue"), stat = "qq") +
  geom_line(data = dairy_queen , aes(sample = cal_fat, color = "red"), stat = "qq") +
  labs(title = "Q-Q Plot Comparison")
```

Comparing these two lines easily shows us the differences between the
simulated normal distribution and the actual distributions. As noted
earlier, the actual distribution has a right skew and based on these
lines cannot be classified as a normal distribution.
...
**Insert your answer here** After looking at the larger simulation of
distributions I think that the distribution in the actual data is nearly
normal. A good way to decide if the plots are similar enough is to ask
yourself "If I wouldn't know which one was the actual data, would I be
able to see which plot doesn't belong?". Using that method, I think it
matches and can be considered normal.
...
Interestingly, while the two fast food chains' distributions originally
looked similar there now seems to be a major difference. Using the same
technique described above, it is pretty easy to spot the plot that looks
significantly different than the others. The actual data does not seem
to be normally distributed. This is caused by the specific outliers
...
```{r}
#calculate probability of sodium being above 2300 mgs (the recommended amount for adults per entire day)

#mcdonalds' factors
mcds_mean <- mean(mcdonalds$sodium)
mcds_sd <- sd(mcdonalds$sodium)

#mcdonalds theoretical normal probability
1 - pnorm(q = 2300, mean = mcds_mean, sd = mcds_sd)

#mcdonalds empirical probability
mcdonalds %>% 
  filter(sodium > 2300) %>%
  summarise(percent = n() / nrow(mcdonalds))

#dairy queens factors
dqs_mean <- mean(dairy_queen$sodium)
dqs_sd <- sd(dairy_queen$sodium)

#dairy queens theoretical normal probability
1 - pnorm(q = 2300, mean = dqs_mean, sd = dqs_sd)

#dairy quenns empirical probability
dairy_queen %>% 
  filter(sodium > 2300) %>%
  summarise(percent = n() / nrow(dairy_queen))
```

## The Dairy Queen sodium probabilities were a lot closer aligned than than the McDonald probabilities. This implies that the Dairy Queen sodium amounts is more evenly distributed that McDonalds. However, since we were dealing with the upper boundaries of the distributions we are more prone to outliers having a larger impact. This is particularly true in the case of Dairy Queen
...
```{r}
ggplot(data = fastfood, aes(x = sodium, color = restaurant)) +
  geom_density()
```

This is too hard to read. It does seem to show a bit more of a bell
curve for Taco Bell. Let's try something else:
...
By using facet_wrap() to divide the graphs it seems to show Burger King
as the most evenly distributed.
...
There should not be a stepwise pattern in a normally distributed plot. I
would assume the reason for the pattern is due to a limited amount of
outliers skewing the line. For whatever reason, once restaurants pass a
certain threshold they seem to just be adding a ton of salt instead of
just a slightly higher amount.
...
Plotting the total carbs of the menu items at Sonic shows us something
interesting. There seems to be a pretty normal distribution until around
the 1.5 quartile at which it gets very skewed. So, the total
distribution is very right skewed.

```{r}
sc_mean <- mean(sonic$total_carb)
sc_sd   <- sd(sonic$total_carb)

ggplot(data = sonic, aes(x = total_carb)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..)) +
        stat_function(fun = dnorm, args = c(mean = sc_mean, sd = sc_sd), col = "red")
```

The previous results can be confirmed with this histogram. The results
are evenly distributed besides for what appears to be a few extreme
outliers heavily skewing right.

```{r}
(sonic$item[sonic$total_carb >= 100])
```

By printing the names of the outliers by we can see that the reason for
the sudden jump in total carbs is due to them being larger servings of
the same food and not an entirely new item. This explains the outliers.
